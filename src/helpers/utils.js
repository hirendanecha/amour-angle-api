...
exports.channelNotificationEmail = async (userData) => {
  let name = userData?.userName;
  let msg = `You have been assign in Amour Aangle channel by the Amour Aangle Admin.
  To access your channel, log into your Amour Aangle account,click on the
  Amour Aangle icon at the top of the page,then click on My Channel.`;
...
exports.communityApproveEmail = async (profileId, isApprove) => {
  ...
  if (userData) {
    ...
    let msg = "";
    if (isApprove === "Y") {
      msg = `Amour Aangle has approved your Connection application.`;
    } else {
      msg = `Amour Aangle has unapproved your Connection application.`;
    }
    let redirectUrl = `${environment.FRONTEND_URL}`;
    const mailObj = {
